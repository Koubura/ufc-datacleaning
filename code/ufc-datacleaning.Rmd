...
ufc_numeric %>% 
  convert(int(col.names))

#is.numeric()


#p_val =ad.test(as.numeric(unlist(ufc_numeric[,3])))$p.value
shapiro.test(as.numeric(unlist(ufc_numeric[,12])))$p.value

for (i in 1:ncol(ufc_numeric)) {
  if (i == 1)cat("Variables que no siguen una distribución normal:\n")
  #if (is.integer(ufc[,i]) |is.numeric(ufc[,i])) {
    p_val =shapiro.test(as.numeric(unlist(ufc_numeric[,i])))$p.value
    #print(p_val)
    if (p_val < alpha) {
      cat(col.names[i])# Format output
      if (i <ncol(ufc_numeric) - 1)cat(", ")
      if (i %% 3 == 0)cat("\n")
    }
  #}
}
...
par(mfrow=c(2,2))
for(i in 1:ncol(ufc_numeric)) {
  if (is.numeric((as.numeric(unlist(ufc_numeric[,i]))))){
    qqnorm((as.numeric(unlist(ufc_numeric[,i]))),main = paste("Normal Q-Q Plot for ",colnames(ufc_numeric)[i]))
    qqline((as.numeric(unlist(ufc_numeric[,i]))),col="red")
    hist((as.numeric(unlist(ufc_numeric[,i]))), 
      main=paste("Histogram for ", colnames(ufc_numeric)[i]), 
      xlab=colnames(ufc_numeric)[i], freq = FALSE)
  }
}
